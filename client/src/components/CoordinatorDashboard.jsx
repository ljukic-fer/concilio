import axios from 'axios';
import React, { useEffect, useState } from 'react'
import { useHistory } from 'react-router-dom/cjs/react-router-dom';
import styles from '../style';
import { useDispatch, useSelector } from 'react-redux';
import { setDob, setEmail, setFirstName, setLastName, setRole } from '../redux/actions/AuthActions';
import ConferenceDetails from './ConferenceDetails';
import ManageParticipants from './ManageParticipants';
import ManageEvents from './ManageEvents';
import Repository from './Repository';
import NewConferenceForm from './NewConferenceForm';

const CoordinatorDashboard = () => {
    const currentUser = localStorage.getItem('email');
    const history = useHistory();
    const dispatch = useDispatch();
    const firstName = useSelector((state) => state.auth.firstName);
    const lastName = useSelector((state) => state.auth.lastName);
    const [newConference, setNewConference] = useState(false);
    const [selectedConference, setSelectedConference] = useState(false);
    const [conferences, setConferences] = useState(null);
    const [participantsScreen, setParticipantsScreen] = useState(false);
    const [eventsScreen, setEventsScreen] = useState(false);
    const [newConferenceScreen, setNewConferenceScreen] = useState(false);
    const [invites, setInvites] = useState([]);

    useEffect(() => {
        const token = localStorage.getItem('token');
        if (token) {
            axios.get('http://localhost:5000/profile', {
                headers: {
                    Authorization: token
                }
            })
                .then(response => {
                    dispatch(setEmail(response.data.email));
                    //dispatch(setFirstName(response.data.firstName));
                    dispatch(setLastName(response.data.lastName));
                    dispatch(setDob(response.data.dob));
                    dispatch(setRole(response.data.role));
                    console.log(response.data)
                })
                .catch(error => {
                    console.error('Error fetching profile:', error);
                    console.log('Error response data:', error.response.data);
                    //history.push('/login');
                });

            axios.get('http://localhost:5000/conferences', {
                headers: {
                    Authorization: token
                }
            })
                .then(response => {
                    setConferences(response.data);
                })
                .catch(error => {
                    console.error('Error fetching conferences:', error);
                });

        } else {
            history.push('/login');
        }
    }, [dispatch]);

    useEffect(() => {
        fetchInvites();
        window.scrollTo(0, 0);
    }, [])


    const handleAcceptInvite = async (invite) => {
        const confirmation = window.confirm('Are you sure you want to attend this conference?');
        if (confirmation) {
            try {
                const token = localStorage.getItem('token');
                const response = await axios.post('http://localhost:5000/attend-conference', {
                    email: localStorage.getItem('email'),
                    conference_id: invite.conference_id
                }, {
                    headers: {
                        Authorization: token
                    }
                });

                alert(response.data.message);
                fetchInvites();  // Refresh invites after attending
            } catch (error) {
                console.error('Error attending conference:', error);
            }
        }
    };

    const fetchInvites = async () => {
        const token = localStorage.getItem('token')
        try {
            const response = await axios.get('http://localhost:5000/invites', {
                headers: {
                    Authorization: token
                }
            });
            setInvites(response.data);
        } catch (error) {
            console.error('Error fetching conference details:', error);
        }
    };


    const handleConferenceBoxClick = (conference) => {
        setSelectedConference(conference);
        setNewConferenceScreen(null);
    };

    const handleBackClick = () => {
        setSelectedConference(null);
        setNewConferenceScreen(null);
    };

    const handleParticipants = () => {
        setParticipantsScreen(true);
    }
    const handleBackToDetails = () => {
        setParticipantsScreen(false);
        setEventsScreen(false);
    }
    const handleEvents = () => {
        setEventsScreen(true);
    }
    const handleCreateConference = () => {
        if (newConferenceScreen) setNewConferenceScreen(false);
        else setNewConferenceScreen(true);
    }

    return (
        <div className={`${styles.defaultBackground} min-h-screen`}>
            {localStorage.getItem('token') && !selectedConference && !participantsScreen && !eventsScreen &&
                <div className='flex'>
                    <div className={`${styles.marginX} ${styles.marginY} flex-col w-1/2`}>
                        <h1 className={styles.heading}>Welcome home, {firstName} {lastName}</h1>
                        <hr class="h-[1px] my-8 border-t-[1px] border-stone-500"></hr>
                        <h2 className={`${styles.heading} mt-6`}>Conferences:</h2>
                        <p className='text-[14px] mb-6 text-slate-700'>Conferences which you are coordinating are marked with an icon</p>
                        <ul className={styles.body}>
                            {conferences && conferences.map(conference => (
                                <li key={conference.conference_id}>
                                    <div className={`${styles.boxBackground} hover:cursor-pointer hover:scale-110 transition delay-150 text-center shadow-2xl hover:outline my-4 mx-12`}
                                        onClick={() => handleConferenceBoxClick(conference)}>
                                        <div className="flex ml-4">
                                            {conference.coordinator === localStorage.getItem('email') &&
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8 mt-1">
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M17.982 18.725A7.488 7.488 0 0 0 12 15.75a7.488 7.488 0 0 0-5.982 2.975m11.963 0a9 9 0 1 0-11.963 0m11.963 0A8.966 8.966 0 0 1 12 21a8.966 8.966 0 0 1-5.982-2.275M15 9.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                                                </svg>}
                                            <h3 className="mr-18 ml-18 font-bold text-center ml-auto mr-auto mb-2 mt-1">{conference.title}</h3>
                                        </div>
                                        <p className='ml-20 mr-20'>{conference.description}</p>
                                        <p>{conference.start_time} - {conference.end_time}</p>
                                    </div>
                                </li>
                            ))}
                        </ul>

                        <h2 className={`${styles.heading} ${styles.marginY}`}>Invites:</h2>
                        <ul className={styles.body}>
                            {invites.map(invite => (
                                <li key={invite.conference_id}>
                                    <div className={`${styles.boxBackground} text-center shadow-2xl hover:outline my-4 mx-12`}>
                                        <h3 className='font-bold'>{invite.title}</h3>
                                        <p>{invite.description}</p>
                                        <p>{invite.start_time} - {invite.end_time}</p>
                                        <div className="flex justify-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                strokeWidth={1.5} stroke="#32ad26" className="w-12 h-12 mr-20 hover:stroke-green-800"
                                                onClick={() => handleAcceptInvite(invite)}
                                            >
                                                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                            </svg>
                                        </div>
                                    </div>
                                </li>
                            ))}
                        </ul>
                    </div>

                    <div className={`${styles.rightSection}`}>
                        {!newConferenceScreen &&
                            <button className="py-1 px-12 bg-blue-600 font-poppins font-medium text-primary outline-none rounded-[10px] ml-auto mr-8 text-white hover:bg-blue-800" onClick={handleCreateConference}>
                                Create new conference?
                            </button>}
                        {newConferenceScreen &&
                            <div>
                                <NewConferenceForm onBackClick={handleCreateConference}/>
                            </div>}
                    </div>

                </div>}
            {selectedConference && !participantsScreen && !eventsScreen &&
                <ConferenceDetails conference={selectedConference} onBackClick={handleBackClick} />
            }
        </div>
    )
}

export default CoordinatorDashboard